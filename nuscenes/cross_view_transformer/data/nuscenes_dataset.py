...
class NuScenesDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, scene_name, scene_record, helper,
                 transform=None, cameras=[[0, 1, 2, 3, 4, 5]],
                 bev={'h': 200, 'w': 200, 'h_meters': 100, 'w_meters': 100, 'offset': 0.0}):
        self.scene_name = scene_name
        self.transform = transform
    ...
    def get_dynamic_objects(self, sample, annotations):
        ...
        result = np.concatenate((segmentation[..., None], center_score[..., None],
                                 center_offset, center_ohw), 2)
    ...
    def __getitem__(self, idx):
        ...
        # ✅ 추가된 부분: 첫 번째 이미지 로딩 및 텐서 변환
        image_path = Path(self.nusc.dataroot) / sample['images'][0]
        image_bgr = cv2.imread(str(image_path))
        image_rgb = cv2.cvtColor(image_bgr, cv2.COLOR_BGR2RGB)
        image_tensor = torch.tensor(image_rgb).permute(2, 0, 1).float() / 255.0  # CxHxW, 0~1 정규화
        ...
        data = Sample(
            ...
            visibility=visibility,
            object_count=object_count,
            image=image_tensor,  # ✅ 추가된 image 키
            **sample
        )
