...
class NuScenesGeneratedDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # transform 후 bev_obj_count 필드가 유지되지 않았다면 복원
        if not hasattr(data, 'bev_obj_count'):
            if 'bev_obj_count' in self.samples[idx]:
                data.bev_obj_count = self.samples[idx]['bev_obj_count']
