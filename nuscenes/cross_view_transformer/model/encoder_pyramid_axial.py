...
class CrossViewSwapAttention(nn.Module):
    def __init__(
        ...
        # 어텐션 레이어를 리스트로 정의
        self.cross_win_attns = nn.ModuleList([
            CrossWinAttention(dim, heads[index], dim_head[index], qkv_bias)
            for _ in range(4) # 최대 반복 횟수 4에 맞춰 미리 생성
        ])
        self.skip = skip
        
        # MLP와 norm도 리스트로 정의하여 반복마다 다른 레이어를 사용
        self.prenorms = nn.ModuleList([norm(dim) for _ in range(4)])
    ...
    def forward(
        ...
        # =========================================================================
        # object_count에 따라 attention 반복 횟수 동적 결정
        # =========================================================================
        if object_count is not None:
            # object_count의 평균을 내어 배치 전체에 대한 반복 횟수 결정
            # object_count가 (b, N) 형태일 수 있으므로 평균을 사용
            avg_object_count = torch.mean(object_count.float())
            
            if avg_object_count < 10:
                num_iterations = 2
            elif avg_object_count < 30:
                num_iterations = 3
            else:
                num_iterations = 4
        else:
            # object_count가 없을 경우 기본 횟수 2회 반복
            num_iterations = 2

        # 첫 번째 어텐션 블록의 초기 입력
        current_query = rearrange(query, 'b n d (x w1) (y w2) -> b n x y w1 w2 d',
                                  w1=self.q_win_size[0], w2=self.q_win_size[1])

        current_key = rearrange(key, 'b n d (x w1) (y w2) -> b n x y w1 w2 d',
                                  w1=self.feat_win_size[0], w2=self.feat_win_size[1])
        
        current_val = rearrange(val, 'b n d (x w1) (y w2) -> b n x y w1 w2 d',
                                  w1=self.feat_win_size[0], w2=self.feat_win_size[1])

        x_skip_first = rearrange(x, 'b d (x w1) (y w2) -> b x y w1 w2 d',
                                 w1=self.q_win_size[0], w2=self.q_win_size[1]) if self.skip else None

        for i in range(num_iterations):
            # local-to-local cross-attention (첫 번째 어텐션)
            query_attended = self.cross_win_attns[i](current_query, current_key, current_val, skip=x_skip_first)
            
            # MLP and Norm
            current_query = rearrange(query_attended, 'b x y w1 w2 d -> b (x w1) (y w2) d')
            current_query = current_query + self.mlps[i](self.prenorms[i](current_query))

            # 두 번째 어텐션 블록 (local-to-global)
            x_skip_second = rearrange(current_query, 'b (x w1) (y w2) d -> b x y w1 w2 d',
                                      w1=self.q_win_size[0], w2=self.q_win_size[1]) if self.skip else None
            
            query_repeated = repeat(current_query, 'b x y d -> b n x y d', n=n)

            # Local-to-global attention requires re-partitioning key and val
            key_global = rearrange(key, 'b n x y w1 w2 d -> b n (x w1) (y w2) d')
            key_global = rearrange(key_global, 'b n (w1 x) (w2 y) d -> b n x y w1 w2 d',
                                  w1=self.feat_win_size[0], w2=self.feat_win_size[1])
            
            val_global = rearrange(val, 'b n x y w1 w2 d -> b n (x w1) (y w2) d')
            val_global = rearrange(val_global, 'b n (w1 x) (w2 y) d -> b n x y w1 w2 d',
                                  w1=self.feat_win_size[0], w2=self.feat_win_size[1])

            query_attended_2 = self.cross_win_attns[i+1](
                rearrange(query_repeated, 'b n (x w1) (y w2) d -> b n x y w1 w2 d',
                          w1=self.q_win_size[0], w2=self.q_win_size[1]),
                key_global, val_global, skip=x_skip_second
            )

            current_query = rearrange(query_attended_2, 'b x y w1 w2 d -> b (x w1) (y w2) d')
            current_query = current_query + self.mlps[i+1](self.prenorms[i+1](current_query))


        query = self.postnorm(current_query)
        query = rearrange(query, 'b H W d -> b d H W')
