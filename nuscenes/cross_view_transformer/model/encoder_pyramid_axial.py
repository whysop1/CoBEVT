...
class CrossViewSwapAttention(nn.Module):
    ...
    def forward(
        ...
        if object_count is not None:
            # Adaptive scale 계산
            # 10 이하 -> 1.0, 10~30 -> 1.2, 30 초과 -> 1.5
            scale_factors = torch.ones_like(object_count, dtype=torch.float, device=x.device)
            scale_factors = torch.where(object_count > 10, torch.tensor(1.2, device=x.device), scale_factors)
            scale_factors = torch.where(object_count > 30, torch.tensor(1.5, device=x.device), scale_factors)
            # 배치 차원에 맞춰 reshape
            scale_factors = scale_factors.view(-1, 1, 1, 1)  # (B,1,1,1)
        else:
            scale_factors = torch.ones(x.shape[0], 1, 1, 1, device=x.device)
        ...
        query = rearrange(
            self.cross_win_attend_1(query, key, val,
                skip=rearrange(x, 'b d (x w1) (y w2) -> b x y w1 w2 d',
                               w1=self.q_win_size[0], w2=self.q_win_size[1]) if self.skip else None),
            'b x y w1 w2 d -> b (x w1) (y w2) d'
        )

        # object_count 기반 scale 적용
        query = query * scale_factors
        ...
        query = rearrange(
            self.cross_win_attend_2(query, key, val,
                skip=rearrange(x_skip, 'b (x w1) (y w2) d -> b x y w1 w2 d',
                               w1=self.q_win_size[0], w2=self.q_win_size[1]) if self.skip else None),
            'b x y w1 w2 d -> b (x w1) (y w2) d'
        )

        query = query * scale_factors  # 두 번째 attention 후에도 적용
